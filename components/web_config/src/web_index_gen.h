...
// Generated 2026-07-13T23:08:52 from docs/config-ui-mockup.html
// Source size: 158951 bytes
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  input[type=color] { width: 100%; height: 42px; padding: 4px; background: var(--card-2); border: 1px solid var(--border); border-radius: 8px; }
  ...
  .output header { all: unset; display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; text-align: left; }
  .output h3 { margin: 0; font-size: 16px; text-align: left; }
  ...
  .status-strip { display: grid; grid-template-columns: 1fr; gap: 8px; margin-top: 10px; }
  .status-chip { border: 1px solid var(--border); border-radius: 999px; padding: 7px 10px; background: rgba(255,255,255,0.035); font: 700 12px/1.25 ui-monospace, "SF Mono", Menlo, monospace; display: inline-flex; align-items: center; justify-content: space-between; gap: 8px; min-height: 32px; }
  .status-strip .status-chip { border-radius: 14px; padding: 9px 12px; display: grid; grid-template-columns: auto minmax(0, 1fr) auto; justify-content: stretch; gap: 10px; min-height: 38px; }
  .status-chip .shape { font-family: inherit; }
  .status-chip .channel { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .status-chip .channel-id { color: var(--fg); }
  .status-chip .channel-name { color: var(--muted); font: 600 11px/1.2 system-ui, sans-serif; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .status-chip .state { text-align: right; white-space: nowrap; }
  .status-chip.good .shape { color: var(--good); }
  .status-chip.warn .shape { color: var(--warn); }
  .status-chip.bad .shape { color: var(--bad); }
  .status-chip.muted .shape { color: var(--muted); }
  .output-state { font-size: 11px; border: 1px solid var(--border); border-radius: 999px; padding: 3px 7px; white-space: nowrap; background: rgba(255,255,255,0.03); }
  .output-state.good { color: var(--good); border-color: rgba(74,222,128,.35); }
  .output-state.warn { color: var(--warn); border-color: rgba(251,191,36,.35); }
  .output-state.bad { color: var(--bad); border-color: rgba(248,113,113,.35); }
  .output-state.muted { color: var(--muted); }
  .output-title { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
  .battery-gauge { margin: 10px 0; border: 1px solid var(--border); border-radius: var(--radius); background: rgba(255,255,255,0.035); padding: 10px; }
  .battery-gauge-head { display:flex; justify-content:space-between; gap:10px; font-size:13px; font-weight:700; }
  .battery-track { height: 12px; border-radius: 999px; background: #0b0d12; border: 1px solid var(--border); overflow: hidden; margin-top: 8px; }
  .battery-fill { height: 100%; width: 0%; background: var(--muted); transition: width 160ms linear, background 160ms linear; }
  .battery-fill.good { background: var(--good); }
  .battery-fill.warn { background: var(--warn); }
  .battery-fill.bad { background: var(--bad); }
  .power-summary-badges { display:flex; flex-wrap:wrap; gap:6px; margin-top:8px; }
  .danger-zone { border-color: rgba(248,113,113,.5); background: rgba(248,113,113,.06); }
  .planning-banner { border: 1px dashed rgba(251,191,36,.6); background: rgba(251,191,36,.08); color: var(--warn); border-radius: 8px; padding: 8px 10px; margin: 8px 0; font-size: 12px; font-weight: 700; }
  .pulse-preview { display: grid; gap: 6px; margin-top: 8px; }
  .pulse-line { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 6px; align-items: center; font: 700 11px/1.2 ui-monospace, "SF Mono", Menlo, monospace; }
  .pulse-line span { border: 1px solid var(--border); border-radius: 7px; padding: 6px; text-align: center; background: rgba(255,255,255,0.035); }
  .pulse-warning { color: var(--bad); font-size: 12px; margin-top: 6px; }
  .rgb-preview { display:flex; align-items:center; gap:10px; margin-top:8px; }
  .rgb-swatch { width: 42px; height: 42px; border-radius: 50%; border: 1px solid var(--border); box-shadow: 0 0 12px rgba(255,255,255,.18); flex: 0 0 auto; }
...
<main>
  ...
  <section class="panel active" data-panel="controller">
    ...
    <div class="card">
      <h2>Pairing</h2>
      <p class="meta">Status: <span id="pair-state">Loading…</span></p>
    ...
    <div class="card">
      ...
      <div id="mac-list" class="mac-list">Loading…</div>
  ...
  <section class="panel" data-panel="outputs">
    <div class="card">
      <h2>Live Output State</h2>
      <p class="meta">Robot output intent from the current setup and live controller/status data.</p>
      <div id="live-output-strip" class="status-strip">
        <span class="status-chip muted"><span class="shape">●</span><span class="channel"><span class="channel-id">M1</span><span class="channel-name">Motor 1</span></span><span class="state">Loading…</span></span>
        <span class="status-chip muted"><span class="shape">●</span><span class="channel"><span class="channel-id">M2</span><span class="channel-name">Motor 2</span></span><span class="state">Loading…</span></span>
        <span class="status-chip muted"><span class="shape">●</span><span class="channel"><span class="channel-id">S1</span><span class="channel-name">Servo 1</span></span><span class="state">Loading…</span></span>
        <span class="status-chip muted"><span class="shape">●</span><span class="channel"><span class="channel-id">S2</span><span class="channel-name">Servo 2</span></span><span class="state">Loading…</span></span>
      </div>
    </div>
    <div class="card">
      <h2>Live Drive Output</h2>
      <p class="meta" id="mock-detail">Waiting for controller…</p>
      <div class="mock-bot">
        <div class="mock-row"><span id="mock-left-label">Left/M1</span><span id="mock-left-dir" class="mock-dir">stop</span><div class="mock-track"><div id="mock-left-bar" class="mock-fill"></div></div><span id="mock-left-pwm" class="meta">0</span></div>
        <div class="mock-row"><span id="mock-right-label">Right/M2</span><span id="mock-right-dir" class="mock-dir">stop</span><div class="mock-track"><div id="mock-right-bar" class="mock-fill"></div></div><span id="mock-right-pwm" class="meta">0</span></div>
        <div class="mock-row"><span id="mock-steer-label">Steering</span><span id="mock-steer-dir" class="mock-dir">center</span><div class="mock-track"><div id="mock-steer-bar" class="mock-fill"></div></div><span id="mock-steer-pwm" class="meta">1500µs</span></div>
      </div>
    </div>
    <div class="card">
...
Drive behavior is configured in Driving Setup.</span></span></h2>
  ...
  <section class="panel" data-panel="settings">
    <div class="card">
      ...
      <div class="kv">
        <span class="k">Mode</span><span class="v" id="wifi-mode">Loading…</span>
        <span class="k">IP</span><span class="v" id="wifi-ip">Loading…</span>
        <span class="k">SSID</span><span class="v" id="wifi-ssid">Loading…</span>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="battery-gauge" aria-label="Battery gauge">
        <div class="battery-gauge-head"><span id="battery-gauge-label">Battery Loading…</span><span id="battery-gauge-state">● Loading…</span></div>
        <div class="battery-track"><div id="battery-gauge-fill" class="battery-fill"></div></div>
      </div>
      <div class="kv">
        <span class="k">Voltage</span><span class="v" id="battery-voltage">Loading…</span>
        <span class="k">Remaining</span><span class="v" id="battery-percent">Loading…</span>
        <span class="k">State</span><span class="v" id="battery-state">Loading…</span>
        <span class="k">Warn threshold</span><span class="v" id="battery-warn-mv">Loading…</span>
        <span class="k">Cutoff</span><span class="v" id="battery-cutoff-mv">Loading…</span>
      </div>
    ...
    <div class="card">
      ...
      <p class="meta">Choose what enabled channels do in GOOD, WARN, and LOW battery states. Changed values are highlighted.</p>
      <div class="power-summary-badges" aria-label="Power behavior defaults">
        <span class="status-chip good"><span class="shape">●</span><span>GOOD</span><span>Allow</span></span>
        <span class="status-chip warn"><span class="shape">▲</span><span>WARN</span><span>Reduce 50%</span></span>
        <span class="status-chip bad"><span class="shape">✕</span><span>LOW</span><span>Disable</span></span>
      </div>
    ...
    <div class="card">
      <h2>Board I/O</h2>
      <p class="meta">Mode button and RGB/RGBW planning controls are shown with their runtime status.</p>
      <div id="settings-board-io"></div>
    </div>
    ...
    <div class="card">
      ...
      <button id="btn-config-upload" class="oled" style="margin-top:8px;">Preview Configuration</button>
      <span id="config-backup-status" class="meta" style="margin-left:8px;"></span>
      <div id="config-import-preview" class="hint" role="status" aria-live="polite" style="display:none; margin-top:10px;"></div>
      <p class="hint">Import always shows a preview before applying. This does not flash firmware or copy WiFi credentials. Use Firmware Update separately if the target board is not already running compatible firmware.</p>
    </div>

    <div class="card">
      <h2>Generic Board Presets</h2>
      <p class="meta">Load a safe starting template into the form, review the channel assignment, then use Save to persist it.</p>
      <label class="field" for="config-preset-select">Board setup</label>
      <select id="config-preset-select"></select>
      <div id="config-preset-summary" class="hint" role="status" aria-live="polite" style="margin-top:8px;"></div>
      <button id="btn-config-preset" class="ghost" style="margin-top:8px;">Load Preset into Form</button>
    </div>
    ...
    <div class="card danger-zone">
      <h2>Danger Zone</h2>
      <p class="meta">Actions here clear, overwrite, or reset robot setup. They ask for confirmation before changing anything.</p>
      <label class="field" for="disconnect-failsafe-select">Disconnect Failsafe Override</label>
      <select id="disconnect-failsafe-select">
        <option value="safe_stop">Safe stop on disconnect / timeout</option>
        <option value="hold_last">DANGER: hold last controller value after disconnect</option>
      </select>
      <p class="hint">Use hold-last only for controlled bench/setup cases. It lets the last sent drive/aux command persist after the controller disconnects instead of forcing neutral.</p>
      <button id="btn-disconnect-failsafe-save" class="ghost danger" style="width:100%; margin-top:8px;">Save Disconnect Failsafe Override</button>
  ...
  <section class="panel" data-panel="about">
    <div class="card">
      <h2>Robot Status</h2>
      <div class="kv">
        <span class="k">Firmware</span><span class="v" id="fw-version">Loading…</span>
        <span class="k">Board</span><span class="v" id="fw-board">Loading…</span>
        <span class="k">Battery</span><span class="v" id="fw-battery">Loading…</span>
      </div>
...
function defaultDriveSetup() {
  return { layout: 'differential', method: 'tank', left_axis: 'LY', right_axis: 'RY', throttle_axis: 'LY', steering_axis: 'LX', drive_motor_output: 'M1', steering_output: 'S1', precision_source: 'NONE', precision_scale_pct: 50, precision_latching: false, left_speed_pct: 100, right_speed_pct: 100, brake_source: 'NONE', invert_steering_source: 'NONE' };
}
function normalizeDriveSetup(drive = {}, legacyMode = state?.drive_mode || 'tank_split') {
  ...
  next.precision_latching = !!next.precision_latching;
  next.left_speed_pct = Math.max(0, Math.min(100, Number(next.left_speed_pct ?? 100)));
  next.right_speed_pct = Math.max(0, Math.min(100, Number(next.right_speed_pct ?? 100)));
  return next;
}
...
const state = {
  ...
  sw1: { short_action: 'pairing', hold_action: 'clear_pair', hold_ms: 5000, double_action: 'none' },
  disconnect_failsafe: 'safe_stop',
...
const OUTPUT_RGB_PATTERNS = [
  ['solid', 'Solid color'],
  ['blink', 'Blink selected color'],
  ['breathe', 'Breathe / pulse selected color'],
  ['rainbow', 'Rainbow cycle'],
  ['chase', 'Chaser using selected color'],
  ['battery', 'Battery status colors'],
  ['controller', 'Controller status colors'],
];
const OUTPUT_RGB_PATTERNS_NEED_COLOR = new Set(['solid', 'blink', 'breathe', 'chase']);
function outputDisplayName(id) {
  ...
  const raw = cfg.display_name || board.display_name || id;
  const name = String(raw).replace(/\s*\/\s*P\d+\s*$/, '');
...
function liveMotorFromMock(id) {
  const out = state.mockDrive || mockDriveOutput(state.drive, state.gp);
  if (id === 'M1') return out?.left || { direction: 'stop', pwm: 0 };
  if (id === 'M2') return out?.right || { direction: 'stop', pwm: 0 };
  return { direction: 'stop', pwm: 0 };
}
function outputStatus(id) {
  const cfg = state.outputs[id] || {};
  const runtime = state.lastStatus?.runtime?.outputs?.[id];
  const statusOut = runtime || state.lastStatus?.outputs?.[id] || {};
  if (runtime) {
    const reason = runtime.blocked_reason || 'none';
    if (reason !== 'none') {
      const labels = { low_battery:'BLOCKED · LOW BATTERY', disconnect:'BLOCKED · DISCONNECTED', failsafe:'BLOCKED · FAILSAFE', arming:'WAITING · ARMING', deadman:'BLOCKED · DEADMAN', disabled:'DISABLED', no_source:'BLOCKED · NO SOURCE' };
      return { cls: reason === 'disabled' || reason === 'no_source' ? 'muted' : (reason === 'arming' ? 'warn' : 'bad'), shape: reason === 'arming' ? '▲' : '✕', text: labels[reason] || `BLOCKED · ${reason.replaceAll('_',' ').toUpperCase()}`, detail: reason };
    }
    if (id === 'M1' || id === 'M2') {
      const dir = Number(runtime.direction) === 1 ? 'FORWARD' : (Number(runtime.direction) === 2 ? 'REVERSE' : 'STOPPED');
      const pct = Math.round(Math.max(0, Math.min(255, Number(runtime.speed)||0)) / 255 * 100);
      return { cls: dir === 'REVERSE' ? 'bad' : (dir === 'FORWARD' ? 'good' : 'muted'), shape: dir === 'REVERSE' ? '▲' : '●', text: dir === 'STOPPED' ? 'STOPPED' : `${dir} ${pct}%`, detail: `${pct}%` };
    }
    if (runtime.pulse_us) return { cls:'good', shape:'●', text:`${runtime.pulse_us}µs${runtime.esc_arm_phase && runtime.esc_arm_phase !== 'inactive' ? ' · '+runtime.esc_arm_phase.toUpperCase() : ''}`, detail:`${runtime.pulse_us}µs` };
    if (typeof runtime.logical === 'boolean') return { cls:runtime.logical?'good':'muted', shape:'●', text:runtime.logical?`ON ${runtime.physical_high?'HIGH':'LOW'}`:`OFF ${runtime.physical_high?'HIGH':'LOW'}`, detail:runtime.logical?'ON':'OFF' };
    return { cls:'muted', shape:'●', text:'READY', detail:'ready' };
  }
  if (id === 'M1' || id === 'M2') {
    const motor = liveMotorFromMock(id);
    const pct = Math.round(Math.max(0, Math.min(255, motor.pwm || 0)) / 255 * 100);
    const dir = motor.direction || 'stop';
    if (!state.gp && pct === 0) return { cls: 'muted', shape: '●', text: 'STOPPED', detail: 'waiting' };
    if (dir === 'stop' || pct === 0) return { cls: 'muted', shape: '●', text: 'STOPPED', detail: '0%' };
    return { cls: dir === 'reverse' ? 'bad' : 'good', shape: dir === 'reverse' ? '▲' : '●', text: `${dir.toUpperCase()} ${pct}%`, detail: `${pct}%` };
  }
  if (id === 'S1' || id === 'S2') {
    const d = normalizeDriveSetup(state.drive || {}, state.drive_mode);
    if (d.layout === 'servo_steering' && d.method !== 'none' && id === d.steering_output && state.gp) {
      const steer = readDriveAxisMock(d.steering_axis, state.gp);
      const pulse = Math.round(1500 + (Math.max(-512, Math.min(511, steer)) / 512) * 500);
      const side = steer < -20 ? 'LEFT' : (steer > 20 ? 'RIGHT' : 'CENTER');
      return { cls: side === 'CENTER' ? 'muted' : 'good', shape: '●', text: `${pulse}µs ${side}`, detail: `${pulse}µs` };
    }
    if (typeof statusOut.logical === 'boolean' || typeof statusOut.physical_high === 'boolean') {
      const logical = !!statusOut.logical;
      const physical = !!statusOut.physical_high;
      return { cls: logical ? 'good' : 'muted', shape: logical ? '●' : '●', text: logical ? `ON ${physical ? 'HIGH' : 'LOW'}` : `OFF ${physical ? 'HIGH' : 'LOW'}`, detail: logical ? 'ON' : 'OFF' };
    }
    if (cfg.purpose === 'rgb_lighting') {
      const pattern = cfg.rgb?.pattern || cfg.rgb_pattern || 'solid';
      return { cls: 'good', shape: '●', text: `${(cfg.protocol || 'rgb').toUpperCase()} ${pattern}`, detail: pattern };
    }
    if (cfg.purpose === 'disabled') return { cls: 'muted', shape: '●', text: 'DISABLED', detail: 'disabled' };
    return { cls: 'muted', shape: '●', text: cfg.protocol ? cfg.protocol.replaceAll('_', ' ') : 'READY', detail: 'ready' };
  }
  if (cfg.config_only) return { cls: 'warn', shape: '▲', text: 'CONFIG ONLY', detail: 'config only' };
  return { cls: 'muted', shape: '●', text: 'READY', detail: 'ready' };
}
function outputStateChip(id) {
  const st = outputStatus(id);
  return el('span', { class: `output-state ${st.cls}`, 'data-output-state': id }, `${st.shape} ${st.text}`);
}
function renderLiveOutputStrip() {
  const root = document.getElementById('live-output-strip');
  if (!root) return;
  root.innerHTML = '';
  for (const id of ['M1','M2','S1','S2']) {
    const st = outputStatus(id);
    const cfg = state.outputs[id] || {};
    const name = cfg.display_name || outputDisplayName(id);
    root.appendChild(el('span', { class: `status-chip ${st.cls}`, 'data-live-output': id }, [
      el('span', { class: 'shape' }, st.shape),
      el('span', { class: 'channel' }, [
        el('span', { class: 'channel-id' }, id),
        el('span', { class: 'channel-name' }, name),
      ]),
      el('span', { class: 'state' }, st.text),
    ]));
  }
}
function updateOutputHeaderStates() {
  for (const id of activeBoardProfile().outputs.map(o => o.id)) {
    const slot = document.querySelector(`[data-output-state-slot="${id}"]`);
    if (!slot) continue;
    slot.innerHTML = '';
    slot.appendChild(outputStateChip(id));
  }
}
function renderMockRobot(out) {
  ...
  const leftLabel = document.getElementById('mock-left-label');
  const rightLabel = document.getElementById('mock-right-label');
  const steerLabel = document.getElementById('mock-steer-label');
  if (leftLabel) leftLabel.textContent = d.layout === 'servo_steering' ? `${d.drive_motor_output} drive` : 'Left/M1';
  if (rightLabel) rightLabel.textContent = d.layout === 'servo_steering' ? 'Other motor' : 'Right/M2';
  if (steerLabel) steerLabel.textContent = d.layout === 'servo_steering' ? `${d.steering_output} steer` : 'Steering';
  ...
  for (const [side, motor] of rows) {
    ...
    if (bar) { bar.style.width = `${Math.max(0, Math.min(255, motor.pwm)) / 255 * 100}%`; bar.classList.toggle('reverse', motor.direction === 'reverse'); }
    if (pwm) pwm.textContent = String(motor.pwm);
  }
  const steer = d.layout === 'servo_steering' && state.gp ? readDriveAxisMock(d.steering_axis, state.gp) : 0;
  const steerPct = Math.abs(steer) / 512 * 100;
  const steerDir = steer < -20 ? 'left' : (steer > 20 ? 'right' : 'center');
  const steerPulse = Math.round(1500 + (Math.max(-512, Math.min(511, steer)) / 512) * 500);
  const steerDirEl = document.getElementById('mock-steer-dir');
  const steerBar = document.getElementById('mock-steer-bar');
  const steerPwm = document.getElementById('mock-steer-pwm');
  if (steerDirEl) steerDirEl.textContent = steerDir;
  if (steerBar) { steerBar.style.width = `${Math.max(0, Math.min(100, steerPct))}%`; steerBar.classList.toggle('reverse', steer < 0); }
  if (steerPwm) steerPwm.textContent = d.layout === 'servo_steering' ? `${steerPulse}µs` : String(Math.round(steer));
  renderLiveOutputStrip();
  updateOutputHeaderStates();
}
...
Precision mode source: optional button/source that enables slow driving. By default it is active only while held; enable latching to toggle precision mode with each press.
...
function renderDriveModeCard() {
  ...
  card.appendChild(el('details', {}, [
    ...
    el('div', { class: 'grid-2' }, [
      ...
      el('label', { class: 'check' }, [(() => { const inp = el('input', { type: 'checkbox' }); inp.checked = !!d.precision_latching; inp.addEventListener('change', e => { d.precision_latching = e.target.checked; }); return inp; })(), el('span', {}, 'Latch precision control (press to toggle)')]),
      ...
      el('div', {}, [el('label', { class: 'field' }, 'Left drive speed %'), numberInput(d.left_speed_pct ?? 100, 0, 100, 1, v => { d.left_speed_pct = v; })]),
      el('div', {}, [el('label', { class: 'field' }, 'Right drive speed %'), numberInput(d.right_speed_pct ?? 100, 0, 100, 1, v => { d.right_speed_pct = v; })]),
      el('p', { class: 'hint' }, 'Straight-line trim: leave both at 100%, then lower only the side that runs faster.'),
...
function ensureRoleDefaults(cfg) {
  ...
  if (!cfg.rgb) cfg.rgb = {
    pattern: cfg.rgb_pattern || 'solid',
    r: cfg.rgb_r ?? 255,
    g: cfg.rgb_g ?? 107,
    b: cfg.rgb_b ?? 53,
    w: cfg.rgb_w ?? 0,
    brightness_pct: cfg.rgb_brightness_pct ?? 40,
    led_count: cfg.rgb_led_count ?? 1,
  };
  if (!cfg.motor_mode) cfg.motor_mode = 'proportional';
  cfg.acceleration_ms = Math.max(0, Math.min(10000, Number(cfg.acceleration_ms ?? cfg.ramp_ms ?? 0)));
  cfg.deceleration_ms = Math.max(0, Math.min(10000, Number(cfg.deceleration_ms ?? 0)));
  cfg.ramp_curve = cfg.ramp_curve === 's_curve' ? 's_curve' : 'linear';
  cfg.ramp_smoothing_pct = Math.max(0, Math.min(100, Number(cfg.ramp_smoothing_pct ?? 50)));
...
const MOTION_PRESETS = {
  instant: { acceleration_ms: 0, deceleration_ms: 0, label: 'Instant' },
  sport:   { acceleration_ms: 150, deceleration_ms: 100, label: 'Sport' },
  medium:  { acceleration_ms: 500, deceleration_ms: 350, label: 'Medium' },
  slow:    { acceleration_ms: 1200, deceleration_ms: 800, label: 'Slow', ramp_curve: 'linear' },
  s_curve: { acceleration_ms: 700, deceleration_ms: 500, label: 'S-Curve', ramp_curve: 's_curve' },
};
function motionPresetFor(cfg) {
  if (cfg.ramp_curve === 's_curve') return 's_curve';
  if (cfg.motion_preset === 'custom') return 'custom';
  return Object.entries(MOTION_PRESETS).find(([, p]) => p.acceleration_ms === cfg.acceleration_ms && p.deceleration_ms === cfg.deceleration_ms)?.[0] || 'custom';
}
...
function pulseCalibrationIssue(pulse) {
  if (!(Number(pulse.min_us) < Number(pulse.center_us) && Number(pulse.center_us) < Number(pulse.max_us))) {
    return 'Pulse order must be min < center < max.';
  }
  return '';
}
function renderPulsePreview(cfg) {
  const p = cfg.pulse || pulseDefaultsFor(cfg.protocol);
  const issue = pulseCalibrationIssue(p);
  return el('div', { class: 'pulse-preview' }, [
    el('div', { class: 'pulse-line' }, [
      el('span', {}, `min ${p.min_us}µs`),
      el('span', {}, `center ${p.center_us}µs`),
      el('span', {}, `max ${p.max_us}µs`),
    ]),
    el('p', { class: issue ? 'pulse-warning' : 'hint' }, issue || `Frame ${p.frame_hz} Hz · neutral deadzone ${p.neutral_deadzone}%`),
  ]);
}
function renderFailsafeSummary(id, cfg) {
  const weapon = !!cfg?.safety?.weapon;
  const disconnect = weapon ? 'Safe state 🔒 forced' : 'Safe state / configured default';
  const low = weapon ? 'Disable 🔒 forced' : POWER_ACTION_LABELS[powerResolvedAction(id, cfg, 'LOW')];
  return el('div', { class: 'summary power-safety' }, [
    el('strong', {}, 'Failsafe: '),
    `disconnect = ${disconnect}; LOW battery = ${low}.`,
  ]);
}
function renderMotionProfile(o, cfg) {
  ensureRoleDefaults(cfg);
  const details = el('details', { class: 'summary', style: 'margin-top:10px' });
  details.appendChild(el('summary', {}, 'Advanced: acceleration and deceleration'));
  const select = el('select');
  for (const [value, label] of [['instant','Instant'],['sport','Sport'],['medium','Medium'],['slow','Slow'],['s_curve','S-Curve'],['custom','Custom']]) {
    const opt = el('option', { value }, label); if (motionPresetFor(cfg) === value) opt.selected = true; select.appendChild(opt);
  }
  const controls = el('div', { class: 'grid-2', style: 'margin-top:8px' });
  const rebuild = () => {
    controls.replaceChildren(el('div', {}, [el('label', { class: 'field' }, 'Motion preset'), select]));
    if (motionPresetFor(cfg) === 's_curve') {
      const smoothing = el('select');
      for (const [value,label] of [[25,'Light'],[50,'Balanced'],[75,'Strong']]) { const opt=el('option',{value:String(value)},label); if (cfg.ramp_smoothing_pct===value) opt.selected=true; smoothing.appendChild(opt); }
      smoothing.addEventListener('change', e => { cfg.ramp_smoothing_pct=Number(e.target.value); });
      controls.appendChild(el('div', {}, [el('label', { class: 'field' }, 'Smoothing'), smoothing]));
    }
    if (motionPresetFor(cfg) === 'custom') {
      controls.appendChild(el('div', {}, [el('label', { class: 'field' }, 'Acceleration time (ms)'), numberInput(cfg.acceleration_ms, 0, 10000, 10, v => { cfg.acceleration_ms = v; })]));
      controls.appendChild(el('div', {}, [el('label', { class: 'field' }, 'Deceleration time (ms)'), numberInput(cfg.deceleration_ms, 0, 10000, 10, v => { cfg.deceleration_ms = v; })]));
    }
  };
  select.addEventListener('change', e => { const p = MOTION_PRESETS[e.target.value]; if (p) { delete cfg.motion_preset; cfg.acceleration_ms = p.acceleration_ms; cfg.deceleration_ms = p.deceleration_ms; cfg.ramp_curve = p.ramp_curve || 'linear'; } else { cfg.motion_preset = 'custom'; cfg.ramp_curve = 'linear'; } rebuild(); });
  details.appendChild(el('p', { class: 'hint' }, 'Acceleration controls ramp-up; deceleration controls commanded slowing. Disconnect and safety stops remain immediate.'));
  details.appendChild(controls); rebuild(); return details;
}
...
function renderPulseControls(o, cfg) {
  ...
  for (const [label, key, min, max, step] of fields) {
    grid.appendChild(el('div', {}, [
      ...
      numberInput(cfg.pulse[key], min, max, step, v => { cfg.pulse[key] = v; renderOutputs(); }),
    ]));
  }
...
function rgbToHex(rgb) {
  ...
  return '#' + [clamp(rgb.r), clamp(rgb.g), clamp(rgb.b)].map(v => v.toString(16).padStart(2, '0')).join('');
}
...
function rgbChannelInput(rgb, key, label) {
  const input = numberInput(rgb[key] ?? 0, 0, 255, 1, v => { rgb[key] = v; renderOutputs(); });
  input.setAttribute('data-rgb-channel', key);
  return el('div', {}, [el('label', { class: 'field' }, label), input]);
}
function renderRgbLightingControls(o, cfg) {
  ensureRoleDefaults(cfg);
  const rgb = cfg.rgb;
  const patternSel = driveSelect(rgb.pattern || 'solid', OUTPUT_RGB_PATTERNS, v => { rgb.pattern = v; renderOutputs(); });
  const color = el('input', { type: 'color', value: rgbToHex(rgb), 'aria-label': 'Pattern color wheel' });
  color.addEventListener('input', e => { Object.assign(rgb, hexToRgb(e.target.value)); renderOutputs(); });
  const white = numberInput(rgb.w ?? 0, 0, 255, 1, v => { rgb.w = v; renderOutputs(); });
  const bright = numberInput(rgb.brightness_pct ?? 40, 0, 100, 1, v => { rgb.brightness_pct = v; renderOutputs(); });
  const ledCount = numberInput(rgb.led_count ?? 1, 1, 300, 1, v => { rgb.led_count = v; renderOutputs(); });
  const needsColor = OUTPUT_RGB_PATTERNS_NEED_COLOR.has(rgb.pattern || 'solid');
  const previewColor = `rgba(${rgb.r ?? 0}, ${rgb.g ?? 0}, ${rgb.b ?? 0}, ${(rgb.brightness_pct ?? 40) / 100})`;
  const rows = [
    el('div', {}, [el('label', { class: 'field' }, 'Pattern'), patternSel]),
    el('div', {}, [el('label', { class: 'field' }, 'LED count'), ledCount, el('p', { class: 'hint' }, 'Number of addressable LEDs in this S1/S2 string (1–300).')]),
    el('div', {}, [el('label', { class: 'field' }, 'Brightness (%)'), bright]),
  ];
  if (needsColor) {
    rows.push(el('div', {}, [el('label', { class: 'field' }, 'Pattern color wheel'), color]));
    rows.push(rgbChannelInput(rgb, 'r', 'Red (0–255)'));
    rows.push(rgbChannelInput(rgb, 'g', 'Green (0–255)'));
    rows.push(rgbChannelInput(rgb, 'b', 'Blue (0–255)'));
    if (cfg.protocol === 'rgbw') rows.push(el('div', {}, [el('label', { class: 'field' }, 'White channel (0–255)'), white]));
  }
  return el('div', { class: 'summary', style: 'margin-top:10px' }, [
    el('strong', {}, 'RGB / RGBW lighting'),
    el('p', { class: 'hint' }, 'Configures this S1/S2 signal output as an addressable lighting channel. Patterns that use a selected color show the color picker.'),
    el('div', { class: 'grid-2', style: 'margin-top:8px' }, rows),
    needsColor ? el('div', { class: 'rgb-preview' }, [
      el('span', { class: 'rgb-swatch', style: `background:${previewColor}` }, ''),
      el('span', { class: 'hint' }, `RGB(${rgb.r ?? 0}, ${rgb.g ?? 0}, ${rgb.b ?? 0})${cfg.protocol === 'rgbw' ? ` W(${rgb.w ?? 0})` : ''}`),
    ]) : el('p', { class: 'hint' }, 'This pattern uses generated/status colors, so no color wheel is needed.'),
  ]);
}
...
function renderOutput(o) {
  ...
  const isRgbLighting = cfg.purpose === 'rgb_lighting';
  const isServo = o.kind === 'servo' && cfg.purpose === 'servo';
  ...
  purposeSel.addEventListener('change', e => {
    ...
    else if (v === 'rgb_lighting') { state.outputs[o.id].protocol = 'rgbw'; state.outputs[o.id].semantics = 'rgb_lighting'; ensureRoleDefaults(state.outputs[o.id]); }
  ...
  const validProtocol = (p) => {
    ...
    if (cfg.purpose === 'rgb_lighting') return p === 'rgb' || p === 'rgbw';
    return false;
  };
  ...
  protocolSel.addEventListener('change', e => { state.outputs[o.id].protocol = e.target.value; if (state.outputs[o.id].purpose === 'servo' || state.outputs[o.id].purpose === 'esc') applyPulseDefaults(state.outputs[o.id], e.target.value); renderOutputs(); });

  card.appendChild(el('header', {}, [
    el('div', { class: 'output-title' }, [el('h3', {}, outputDisplayName(o.id)), el('span', { 'data-output-state-slot': o.id }, outputStateChip(o.id))]),
    el('span', { class: 'id' }, o.id),
  ]));
  if (o.detail) card.appendChild(el('p', { class: 'hint' }, o.detail));
  if (o.configOnly || cfg.config_only) card.appendChild(el('div', { class: 'planning-banner' }, 'Header H1 — config/planning only. Not active at runtime yet.'));
  ...
  if (o.runtimeControlled) {
    ...
    if (drive.method === 'none') {
      ...
      card.appendChild(el('div', { class: 'grid-2', style: 'margin-top:10px' }, [
        el('div', {}, [el('label', { class: 'field' }, 'Channel name'), nameInput]),
        el('div', {}, [el('label', { class: 'field' }, 'PWM frequency (Hz)'), numberInput(pwm.frequency_hz || 20000, 1000, 40000, 100, v => { pwm.frequency_hz = v; })]),
        el('label', { class: 'check' }, [(() => { const inp = el('input', { type: 'checkbox' }); inp.checked = cfg.direction === 'reversed'; inp.addEventListener('change', e => { cfg.direction = e.target.checked ? 'reversed' : 'normal'; }); return inp; })(), el('span', {}, `Invert ${o.id} motor direction`)]),
      ]));
      card.appendChild(renderRuntimeDriveAssignment(o));
    }
    card.appendChild(renderFailsafeSummary(o.id, cfg));
    return card;
  }

  if (isRgbLighting) {
    card.appendChild(renderRgbLightingControls(o, cfg));
    card.appendChild(renderFailsafeSummary(o.id, cfg));
    return card;
  }

  if (cfg.purpose === 'disabled') {
    card.appendChild(el('div', { class: 'summary', style: 'margin-top:10px' }, [
      el('strong', {}, 'Disabled output'),
      el('p', { class: 'hint' }, 'This channel is unused. No controller source, direction, deadzone, or secondary input is applied.'),
    ]));
    card.appendChild(renderFailsafeSummary(o.id, cfg));
    return card;
  }
  ...
  if (isDigitalOutput) {
    card.appendChild(el('div', { class: 'grid-2', style: 'margin-top:10px' }, [
      el('div', {}, [
        el('label', { class: 'field' }, 'Control source'),
        primary,
      ]),
      el('div', {}, [
  ...
  if (isDigitalOutput) {
    ...
    grid2.appendChild(el('div', {}, [
      el('p', { class: 'hint' }, 'Servo uses only the primary input. Secondary/reverse input is only shown for ESC / motor controller outputs.'),
    ]));
  } else {
    grid2.appendChild(el('div', {}, [
  ...
  if (cfg.purpose === 'drive' || cfg.purpose === 'servo' || cfg.purpose === 'esc') {
    card.appendChild(renderMotionProfile(o, cfg));
  }
...
function normalizeSw1Config(cfg = {}) {
  return {
    short_action: cfg.short_action || 'pairing',
    double_action: cfg.double_action || 'none',
    hold_action: cfg.hold_action || cfg.long_action || 'clear_pair',
    hold_ms: Number(cfg.hold_ms ?? cfg.long_hold_ms ?? 5000),
  };
}
function renderSw1Controls() {
  const cfg = state.sw1 = normalizeSw1Config(state.sw1);
  const hold = numberInput(cfg.hold_ms || 5000, 1000, 10000, 500, v => { cfg.hold_ms = v; });
  return el('div', { class: 'summary', style: 'margin-top:10px' }, [
    el('strong', {}, 'SW1 timing/actions'),
    el('p', { class: 'hint' }, 'Firmware-backed physical mode button actions. Short press waits briefly so a second press can become a double press.'),
    el('div', { class: 'grid-2', style: 'margin-top:8px' }, [
      ...
      el('div', {}, [el('label', { class: 'field' }, 'Hold action'), sw1ActionSelect(cfg.hold_action, v => { cfg.hold_action = v; })]),
      el('div', {}, [el('label', { class: 'field' }, 'Hold time (ms)'), hold]),
    ]),
    el('p', { class: 'hint' }, 'Default: short press starts pairing; hold for 5000 ms clears controllers and starts pairing.'),
  ]);
}
...
function renderRgbControls() {
  ...
  return el('div', { class: 'summary', style: 'margin-top:10px' }, [
    ...
    el('div', { class: 'planning-banner' }, 'RGB/RGBW — planning only until firmware pattern runtime is active.'),
    el('div', { class: 'grid-2', style: 'margin-top:8px' }, [
...
function renderSettingsBoardIo() {
  const root = document.getElementById('settings-board-io');
  if (!root) return;
  const profile = activeBoardProfile();
  root.innerHTML = '';
...
function rememberPowerOpenState() {
  document.querySelectorAll('details[data-power-card]').forEach(d => {
    if (d.open) powerOpenCards.add(d.dataset.powerCard);
    else powerOpenCards.delete(d.dataset.powerCard);
  });
}
function renderPowerCard(id) {
  ...
  if (powerOpenCards.has(id)) details.open = true;
  details.addEventListener('toggle', () => {
    if (details.open) powerOpenCards.add(id);
    else powerOpenCards.delete(id);
  });
...
function editableOutputPatch(outputs) {
  const patch = { drive_mode: state.drive_mode, drive: normalizeDriveSetup(state.drive || {}, state.drive_mode), sw1: normalizeSw1Config(state.sw1), disconnect_failsafe: state.disconnect_failsafe || 'safe_stop' };
  ...
  for (const [id, cfg] of Object.entries(outputs)) {
    ...
    patch[id] = {
      ...
      acceleration_ms: cfg.acceleration_ms,
      deceleration_ms: cfg.deceleration_ms,
      ramp_curve: cfg.ramp_curve,
      ramp_smoothing_pct: cfg.ramp_smoothing_pct,
...
function configBackupPayload() {
  return {
    ...
    local_ui_only: { H1: state.outputs.H1, rgb: state.rgb },
    battery: {
...
function patchFromUploadedConfig(data) {
  ...
  if (data.schema !== undefined && data.schema !== CONFIG_SCHEMA) {
    throw new Error(`incompatible schema: ${String(data.schema)}`);
  }
...
function applyUploadedPatchToForm(patch) {
  ...
  if (patch.sw1 && typeof patch.sw1 === 'object') state.sw1 = normalizeSw1Config(patch.sw1);
  if (patch.disconnect_failsafe === 'hold_last' || patch.disconnect_failsafe === 'safe_stop') state.disconnect_failsafe = patch.disconnect_failsafe;
  for (const id of activeBoardProfile().outputs.map(o => o.id)) {
...
const CONFIG_SCHEMA = 'combat-robot-controller-v2.config.v1';
const GENERIC_BOARD_PRESETS = {
  tank_drive: { label: 'Tank drive', summary: 'M1 = left stick, M2 = right stick; S1/S2 disabled.', drive_mode: 'tank_split', drive: { layout:'differential', method:'tank', left_axis:'LY', right_axis:'RY' } },
  arcade_drive: { label: 'Arcade drive', summary: 'M1/M2 mixed from left-stick throttle and steering; S1/S2 disabled.', drive_mode: 'arcade', drive: { layout:'differential', method:'arcade', throttle_axis:'LY', steering_axis:'LX' } },
  servo_steer: { label: 'Servo-steer car', summary: 'M1 = traction motor, S1 = steering servo; unused outputs disabled.', drive_mode: 'servo_steering', drive: { layout:'servo_steering', method:'servo_steering', throttle_axis:'LY', steering_axis:'LX', drive_motor_output:'M1', steering_output:'S1' } },
  safety_esc: { label: 'Arcade drive + ESC (weapon safety)', summary: 'M1/M2 arcade drive; S1 = OneShot125 ESC with weapon safety; S2 disabled.', drive_mode: 'arcade', drive: { layout:'differential', method:'arcade', throttle_axis:'LY', steering_axis:'LX' }, aux:'weapon' },
  solenoid_accessory: { label: 'Solenoid / accessory', summary: 'Drive mixer disabled; M1 = momentary A-button accessory; other outputs disabled.', drive_mode: 'none', drive: { layout:'differential', method:'none' }, manual:'solenoid' },
  led_strip: { label: 'LED status strip', summary: 'Drive mixer disabled; S1 = RGB status strip; motor outputs and S2 disabled.', drive_mode: 'none', drive: { layout:'differential', method:'none' }, aux:'led' },
  bench_safe: { label: 'Bench-safe (all outputs disabled)', summary: 'No mixer and all four outputs disabled. Safe starting point for USB/API setup.', drive_mode: 'none', drive: { layout:'differential', method:'none' } },
};

function disabledPresetOutput(id) {
  const motor = id === 'M1' || id === 'M2';
  return { ...JSON.parse(JSON.stringify(state.outputs[id] || {})), display_name: id, purpose: motor ? 'drive' : 'disabled', protocol:'none', semantics:'none', primary:'NONE', secondary:'NONE', motor_mode:'disabled', weapon_safety:false };
}
function presetPatch(name) {
  const preset = GENERIC_BOARD_PRESETS[name];
  if (!preset) throw new Error('unknown preset');
  const patch = { drive_mode:preset.drive_mode, drive:normalizeDriveSetup(preset.drive, preset.drive_mode), disconnect_failsafe:'safe_stop' };
  for (const id of ['M1','M2','S1','S2']) patch[id] = disabledPresetOutput(id);
  if (name === 'tank_drive' || name === 'arcade_drive' || name === 'safety_esc') {
    patch.M1 = { ...patch.M1, display_name:'Left drive', purpose:'drive', primary:name === 'tank_drive' ? 'LY' : 'NONE', motor_mode:'proportional' };
    patch.M2 = { ...patch.M2, display_name:'Right drive', purpose:'drive', primary:name === 'tank_drive' ? 'RY' : 'NONE', motor_mode:'proportional' };
  } else if (name === 'servo_steer') {
    patch.M1 = { ...patch.M1, display_name:'Drive motor', purpose:'drive', motor_mode:'proportional' };
    patch.S1 = { ...patch.S1, display_name:'Steering servo', purpose:'servo', protocol:'rc_servo_pwm', semantics:'position_servo', primary:'LX', servo_mode:'bi' };
  } else if (name === 'solenoid_accessory') {
    patch.M1 = { ...patch.M1, display_name:'Solenoid', purpose:'drive', motor_mode:'momentary', primary:'A' };
  }
  if (preset.aux === 'weapon') patch.S1 = { ...patch.S1, display_name:'Safety ESC', purpose:'esc', protocol:'oneshot125', semantics:'esc_forward_only', primary:'RT', weapon_safety:true, failsafe:'neutral', weapon_mode:'hold_to_arm', arming_source:'L1', deadman_source:'R1' };
  if (preset.aux === 'led') patch.S1 = { ...patch.S1, display_name:'Status LEDs', purpose:'rgb_lighting', protocol:'rgb', semantics:'none', rgb_pattern:'battery', rgb_r:0, rgb_g:64, rgb_b:255, rgb_w:0, rgb_brightness_pct:40, rgb_led_count:8 };
  return patch;
}
function configImportSummary(data, patch) {
  const outputIds = ['M1','M2','S1','S2'].filter(id => patch[id]);
  const extras = [];
  if (patch.drive || patch.drive_mode) extras.push('drive setup');
  if (patch.sw1) extras.push('SW1');
  if (patch.disconnect_failsafe) extras.push(`failsafe=${patch.disconnect_failsafe}`);
  if (data.battery) extras.push('battery');
  if (data.max_paired !== undefined) extras.push(`max paired=${data.max_paired}`);
  return `Outputs: ${outputIds.join(', ') || 'none'}${extras.length ? ' · ' + extras.join(' · ') : ''}`;
}

const presetSelect = document.getElementById('config-preset-select');
for (const [id, preset] of Object.entries(GENERIC_BOARD_PRESETS)) presetSelect.appendChild(el('option', { value:id }, preset.label));
function renderPresetSummary() { document.getElementById('config-preset-summary').textContent = GENERIC_BOARD_PRESETS[presetSelect.value].summary; }
presetSelect.addEventListener('change', renderPresetSummary);
renderPresetSummary();
document.getElementById('btn-config-preset').addEventListener('click', () => {
  const preset = GENERIC_BOARD_PRESETS[presetSelect.value];
  applyUploadedPatchToForm(presetPatch(presetSelect.value));
  toast(`${preset.label} loaded — review and Save to persist`);
});
...
document.getElementById('btn-config-upload').addEventListener('click', async () => {
  ...
  try {
    ...
    const patch = patchFromUploadedConfig(data);
    preview.textContent = configImportSummary(data, patch);
    preview.style.display = 'block';
    status.textContent = 'preview ready';
    if (!confirm(`Apply this configuration?\n\n${preview.textContent}`)) {
      status.textContent = 'previewed — not applied';
      return;
    }
...
document.getElementById('disconnect-failsafe-select').addEventListener('change', e => {
  state.disconnect_failsafe = e.target.value === 'hold_last' ? 'hold_last' : 'safe_stop';
});
document.getElementById('btn-disconnect-failsafe-save').addEventListener('click', async () => {
  const selected = document.getElementById('disconnect-failsafe-select').value === 'hold_last' ? 'hold_last' : 'safe_stop';
  if (selected === 'hold_last' && !confirm('DANGER: hold-last keeps the last sent controller value active after disconnect/timeout. Enable only for controlled setup or bench use?')) return;
  try {
    state.disconnect_failsafe = selected;
    await apiPostJSON('/api/config', { disconnect_failsafe: selected });
    toast(selected === 'hold_last' ? 'Disconnect override: HOLD LAST' : 'Disconnect failsafe: safe stop');
  } catch (e) { toast('Failsafe save failed: ' + e.message, 'err'); }
});
...
function applyStatus(s) {
  ...
  const gaugeLabel = document.getElementById('battery-gauge-label');
  const gaugeState = document.getElementById('battery-gauge-state');
  const battCls = battState === 3 ? 'bad' : (battState === 2 ? 'warn' : (battState === 1 ? 'good' : ''));
  if (gaugeFill) {
    gaugeFill.style.width = battMv ? `${Math.max(0, Math.min(100, battPct))}%` : '0%';
    gaugeFill.className = `battery-fill ${battCls}`.trim();
  }
  if (gaugeLabel) gaugeLabel.textContent = battMv ? `Battery ${battPct}% · ${(battMv/1000).toFixed(2)} V / ${battCells}S` : 'Battery Loading…';
  if (gaugeState) {
    gaugeState.textContent = battState === 3 ? '✕ LOW' : (battState === 2 ? '▲ WARN' : (battState === 1 ? '● GOOD' : '● Loading…'));
    gaugeState.className = battCls ? battCls : '';
  }
  renderLiveOutputStrip();
  updateOutputHeaderStates();
  ...
  const boardText = s.board_rev ? `v${s.board_rev}` : (s.board_rev_override ? `v${s.board_rev_override} override` : 'v2 (compile-time)');
  document.getElementById('board-rev').textContent = boardText;
  document.getElementById('fw-board').textContent = boardText;
...
async function loadConfig() {
  try {
    ...
    if (j.sw1 && typeof j.sw1 === 'object') state.sw1 = normalizeSw1Config(j.sw1);
    if (j.disconnect_failsafe === 'hold_last' || j.disconnect_failsafe === 'safe_stop') {
      state.disconnect_failsafe = j.disconnect_failsafe;
      const fs = document.getElementById('disconnect-failsafe-select');
      if (fs) fs.value = state.disconnect_failsafe;
    }
