...
<main>
  ...
  <section class="panel" data-panel="settings">
    ...
    <div class="card">
      ...
      <p class="meta" style="margin:8px 0;">Default is 3S at 33%, matching the old ~3.60 V/cell cutoff. Range: 2–6 cells, 0–80%.</p>
...
function assignedSourceOwners(exceptOutputId) {
  ...
  for (const out of activeBoardProfile().outputs) {
    ...
    for (const src of [cfg.primary, cfg.secondary]) {
      if (src && src !== 'NONE') owners.set(src, out.display_name);
    }
  }
  return owners;
}
...
function applyStatus(s) {
  ...
  state.battery = { mv: battMv, pct: battPct, state: battState, cell_count: battCells, cutoff_percent: battCutoffPct, cutoff_mv: battCutoffMv };
  const batteryText = battMv ? `${(battMv/1000).toFixed(2)} V · ${battPct}% · state ${battState}` : '—';
  ...
  document.getElementById('battery-state').textContent = battState ? String(battState) : '—';
  ...
  if (cellInput && document.activeElement !== cellInput) cellInput.value = String(battCells);
  if (cutoffInput && document.activeElement !== cutoffInput) cutoffInput.value = String(battCutoffPct);
